...
.DESCRIPTION
    ...
    On Remote Desktop Connection Broker servers this will impact all servers in the RD Deployment. This behavior
    can be prevented by using the -IgnoreRDSession switch parameter.
.NOTES
...
.EXAMPLE
    ...
    Invoke-LogoffDisconnectedAdmins -IgnoreRDSession
#>
[CmdletBinding()]
param (
    # Switch to prevent logoff via RDUserSession on Connection Brokers and force local session detection
    [switch]$IgnoreRDSession
)
...
if ($isConnectionBroker -and (-NOT $IgnoreRDSession)) {
